...
ImaginAIは、AI画像生成APIを専用GUIで操作するデスクトップアプリケーションです。Google Gemini API（Image Generation）に対応し、将来的に他のAI画像生成APIの追加にも対応できる拡張性を備えています。

主な機能:

- **画像生成**: モデル選択、アスペクト比、解像度、生成枚数、出力フォーマット、セーフティフィルターなどのパラメータをGUIで設定し、テキストプロンプトから画像を生成
- **参照画像添付**: ファイル選択・ドラッグ＆ドロップ・履歴からの追加による画像参照生成（Image-to-Image）
- **生成履歴管理**: 生成画像のサムネイル一覧表示、検索、パラメータ復元、名前をつけて保存、一括ZIP圧縮エクスポート
- **画像ビューア**: モードレスウィンドウで複数画像を同時に閲覧
