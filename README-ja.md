...
ImaginAIは、AI画像・動画・音楽・音声生成APIを専用GUIで操作するデスクトップアプリケーションです。Google Gemini API（Image Generation）、Veo 3.1（Video Generation）、Lyria 3（Music Generation）、Gemini TTS（Speech Generation）に対応し、将来的に他のAI生成APIの追加にも対応できる拡張性を備えています。
...
- **音声生成（TTS）**: Gemini TTSモデルによるテキスト読み上げ。スタイルプリセットと30種の事前定義ボイスを選択可能。出力はMP3、同梱の音声エンコーダが利用できない場合はWAVに自動フォールバックするため、API呼び出しが成功している限り結果が無駄になりません
- **参照画像添付**: ファイル選択・ドラッグ＆ドロップ・履歴からの追加による画像参照生成（Image-to-Image / Image-to-Video / Image-to-Music）
- **生成履歴管理**: 生成した画像・動画・音楽・音声のサムネイル一覧表示、検索、パラメータ復元、名前をつけて保存、一括ZIP圧縮エクスポート
